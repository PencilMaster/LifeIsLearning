// I had an error and added too many dp[] values when k was smaller than distinct.
// This is fixed, but TLE still holds.
//
//
...
int possibleStringCount(const char* word, const int k) {
    ...
    int result = 0;
    // start from larger, k or distinct, that we have all characters
    int startIterator = (k > distinct ? k : distinct);
    // FOR LOOP OVER DP ARRAY TO ADD ALL >= K LENGTH
    for (size_t resSumIterator = startIterator; resSumIterator < wordSize+1; ++resSumIterator) {
