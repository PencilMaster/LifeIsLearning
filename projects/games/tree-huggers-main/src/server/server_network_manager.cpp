...
void ServerNetworkManager::read_message(sockpp::tcp_socket socket, const std::function<void(const std::string&,
    ...
    while ((count = socket.read(buffer, sizeof(buffer))) > 0) {
        try {
            ...
            std::cerr << "Your move was fine. Your game will stop freezing once the next player has finished their move. \n Error while reading message from " << socket.peer_address() << "\n" << e.what() << std::endl;
        }
    }
    if (count <= 0) {
...
void ServerNetworkManager::handle_incoming_message(const std::string& msg, const sockpp::tcp_socket::addr_t& peer_address) {
    try {
        ...
        ClientRequest* req = ClientRequest::from_json(req_json);
        ...
        ServerResponse* res = RequestHandler::handle_request(*req);
        delete req;
...
#ifdef PRINT_NETWORK_MESSAGES
        std::cout << "Sending response : " << res_msg.substr(0, 200) << std::endl;
        std::cout << " ******************** \n\n";
#endif
...
void ServerNetworkManager::on_player_left(std::string player_id) {
    _rw_lock.lock();
...
void ServerNetworkManager::send_message_to_player(ServerResponse&msg, const Player *player) {
    ...
    try{
        send_message(msg_string, _player_id_to_address.at(player->get_player_id()));
    } catch (std::exception& e) {
        std::cerr << "Encountered error when sending ServerResponse " << e.what() << std::endl;
    }
}
...
void ServerNetworkManager::broadcast_message(ServerResponse &msg, const std::vector<Player *> &players,
...
#ifdef PRINT_NETWORK_MESSAGES
    std::cout << "Broadcasting message : " << msg_string.substr(0, 26) + msg_string.substr(msg_string.find("\"cell_id\":1"), 500) << std::endl;
#endif
    ...
    try {
        for(auto& player : players) {
            if (player != exclude) {
                int nof_bytes_written = send_message(msg_string, _player_id_to_address.at(player->get_player_id()));
            }
        }
    } catch (std::exception& e) {
